		...
		return this.each(function() {	
			...
			$(this)[0].addEventListener('loadedmetadata', function() {
				...
				for (var i = 0; i < settings.content.length; i++) {
					cue = settings.content[i];
					// var left = settings.content[i].second * (duration/100) * ((progressWidth+btnWidth)/100);
					// divs += '<div class="cue" id=cue'+cue.id+' data-cue-id=' + cue.id + ' style="left:' + left + 'px"></div>';
					divs += '<input type="range" value=' + $settings.content[i].second + ' style="width:' + progressWidth + 'px" min="0" max="' + duration + '" />'
				};
